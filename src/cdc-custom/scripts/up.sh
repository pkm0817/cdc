...
── 기동 완료 ─────────────────────────────────────
  source DB   : localhost:58432 (sourcedb / postgres:postgres)
  target DB   : localhost:58433 (targetdb / postgres:postgres)
  sync-service: http://localhost:58280/actuator/health
  Prometheus  : http://localhost:58290
  Grafana     : http://localhost:58300  (admin/admin)
EOS
