...
-- 파이프라인 진행 지점
--
-- go-pq-cdc 는 오프셋 파일을 쓰지 않는다. "어디까지 처리했는지"는 오직
-- 복제 슬롯의 confirmed_flush_lsn 하나에만 남는다. Debezium 판보다 단순하지만
-- 그만큼 취약하다 — 슬롯이 사라지면 진행 지점이 통째로 사라지고, 라이브러리는
-- CreateIfNotExists 설정에 따라 조용히 새 슬롯을 만들어 버린다.
--
-- 그래서 진행 지점을 수신 측 DB 에도 남긴다. 이 표가 있으면 기동할 때
-- 슬롯의 restart_lsn 과 대조해 "처음부터 다시 읽는 것"과 "구간을 건너뛴 것"을
-- 구분할 수 있다. Java 판에서는 보조 수단이었지만, 여기서는 유일한 탐지 수단이다.
-- ============================================================================
...
-- 그걸 어디에도 남기지 않으면 사후에 물을 수 없다. 남길 곳은 지표가 아니라 표다 —
-- 변경 필드명을 Prometheus 레이블로 쓰면 시계열이 (테이블 × 컬럼 조합) 수만큼 생긴다.
-- 지표로는 "몇 건 남겼는가"만 낸다 (cdc_change_audit_rows_total{table}).
--
...
--
-- Java 판과 같은 모양이다. 어느 표를 남길지는 CDC_AUDIT_CHANGED_FIELDS 가 정한다.
--
...
CREATE TABLE cdc_change_audit (
    ...
    source_lsn        BIGINT      NOT NULL,
    source_ts         TIMESTAMPTZ NOT NULL,   -- 원천이 보낸 시각 (적재 시각이 아니다)
    identifiable      BOOLEAN     NOT NULL,
    changed_fields    TEXT        NOT NULL,   -- 쉼표 구분 컬럼명
    unreadable_fields TEXT,                   -- 한쪽 이미지에 실려 오지 않아 판정 못 한 컬럼 (V5)
